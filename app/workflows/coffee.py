...
ASK_COFFEE_TYPE_TEXT = "Какой кофе ты хочешь: горячий или холодный? С сиропом или без?"
TG_WANTS_DIALOG = "dialog:домашний помощник:tg_ask_sonya_wants_coffee"
TG_TYPE_DIALOG = "dialog:домашний помощник:tg_ask_sonya_coffee_type"
...
class CoffeeWorkflow:
    ...
    async def handle_wants_answer(self, answer: SonyaAnswer) -> None:
        ...
        if intent == "YANDEX.REJECT" or any(word in normalized for word in NEGATIVE_WORDS):
            await self._edit_active_or_send("Соня отказалась от кофе.", delete_only())
        ...
        if intent == "YANDEX.CONFIRM" or any(word in normalized for word in POSITIVE_WORDS):
            await self._ha.play_media(self._settings.bedroom_player_entity, ASK_COFFEE_TYPE_TEXT, TG_TYPE_DIALOG)
            await self._edit_active_or_send("Соня хочет кофе. Уточняю, какой именно...", delete_only())
            return

        shown = text or "пустой ответ"
        self._latest_coffee_type = normalize_coffee_answer(shown)
        await self._edit_active_or_send(
            f"Соня ответила: {shown}. Включить кофемашину?",
            confirmation(),
            coffee_type=self._latest_coffee_type,
        )

    async def handle_type_answer(self, answer: SonyaAnswer, *, direct: bool) -> None:
        text = answer.answer.strip()
        coffee_type = normalize_coffee_answer(text)
        self._latest_coffee_type = coffee_type
        LOGGER.info("Sonya coffee type received: dialog=%s direct=%s answer=%r", answer.dialog, direct, text)
        if direct:
            self._set_active_message(self._settings.telegram_admin_chat_id, None)
        await self._edit_active_or_send(
            f"Соня попросила: {coffee_type}. Включить кофемашину?",
            confirmation(),
    ...
    async def confirm_turn_on(self, chat_id: int, message_id: int | None) -> int | None:
        ...
        edited_id = await self._telegram_messages.safe_edit(
            chat_id,
            message_id,
            f"Я включила кофемашину. Соня попросила: {coffee_type}.",
            delete_only(),
        )
    ...
    async def confirm_decline(self, chat_id: int, message_id: int | None) -> int | None:
        ...
        edited_id = await self._telegram_messages.safe_edit(
            chat_id,
            message_id,
            f"Я не включаю кофемашину. Соня попросила: {coffee_type}.",
            delete_only(),
        )
    ...
    async def _remind_later(self, reminder_id: int, reminder: Reminder) -> None:
        try:
            ...
            message_id = await self._telegram_messages.safe_send(
                reminder.chat_id,
                f"Напоминание: Соня просила кофе: {coffee_type}. Включить кофемашину?",
    ...
    async def _edit_active_or_send(self, text: str, reply_markup, coffee_type: str | None = None) -> None:  # noqa: ANN001
