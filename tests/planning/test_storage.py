...
class PlanningStorageTests(unittest.TestCase):
    ...
    def test_empty_database_migrates_repeatably_and_has_required_pragmas(self) -> None:
        ...
        self.assertEqual(self.database.migrate(), 7)
        ...
        self.assertEqual(self.database.connection.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0], 7)
    ...
    def test_concurrent_empty_database_initialization_is_safe(self) -> None:
        ...
        self.assertEqual(versions, [7, 7])
        check = PlanningDatabase(concurrent_path)
        try:
            self.assertEqual(check.connection.execute("SELECT COUNT(*) FROM schema_migrations").fetchone()[0], 7)
        finally:
            check.close()
